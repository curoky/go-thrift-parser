...
# for llvm: add -stdlib=libc++ -fcoroutines-ts
...
iwyu_tool.py -j 32 -p "$EXEC_ROOT" "$(bazel info workspace)" -- \
...
fix_includes.py --comments -b --reorder -p "$EXEC_ROOT" --only_re="blink" <iwyu.out
# fix_includes.py --comments -b --reorder -p "/shm/cache/bazel/execroot/com_github_curoky_blink" <iwyu.out
